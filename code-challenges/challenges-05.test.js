...
const templateWithJQuery = () => {
  // Copy template
  // Insert Charcter Info
  // Append template to the DOM

  const $template = $('#template');
  starWarsPeople.forEach(value => {
    let $resistance = $template.clone();
    // console.log("TEMPLATE COPY", $template)
    $resistance.children('h2').text(value.name);
    $resistance.children('h3').text(value.height);
    $resistance.children('p').text(value.eye_color);
    // console.log("RESISTANCE", $resistance)
    $('main').append($resistance);
  })
}
...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(food => {
    let sliced = food.slice(food.indexOf(" "));
    let slicedAgain = sliced.slice(sliced.indexOf(" ", 2)+1);
    result.push(slicedAgain);
  })
  return result;
};
